...
|----------|--------|-------|--------|
| Edge index | `edge_index.json` | GNN message passing | ✅ Working |
| Node adjacency | Derived from edges | Valid candidate filtering | ✅ Working |
| Node mapping | `node_mapping.json` | Node ID ↔ index | ✅ Working |
| Graph embeddings | `TreeGraphEncoder` | Node representations | ✅ Working |

### Need to Implement

| Variable | Source | Usage | Priority |
|----------|--------|-------|----------|
| Node type | PoB `tree.lua` → `type` field | Weight keystone vs small node | HIGH |
...
**Data Flow:**
```
edge_index.json → torch.tensor([2, 431]) → GCNConv layers → node embeddings [412, 128]
                                                ↓
...
| Variable | Source | Usage | Status |
|----------|--------|-------|--------|
| Fixed node features | Random init | GNN input (placeholder) | ⚠️ Dummy data |

### Need to Implement

| Variable | Source | Usage | Priority |
|----------|--------|-------|----------|
| Flat life/ES | PoB `tree.lua` → `sd` field | Defense calculation | **CRITICAL** |
...
**Data Flow:**
```
PoB tree.lua → parse node stats → node_stats dict
                                         ↓
                            allocated_nodes [N] → aggregate stats → build_stats dict
                                                                           ↓
                                                                    Used in reward()
```
...
| Variable | Source | Usage | Status |
|----------|--------|-------|--------|
| Allocated nodes set | PathGenerator | Prevent re-allocation | ✅ Working |
| Current context | Concatenated tensor | Node selection scoring | ✅ Working |
...
Initial state (base_life, 0 DPS, 0 res)
        ↓
   [Select node 1] → update_state() → {life: 150, dps: 0, fire_res: 0}
        ↓
   [Select node 2] → update_state() → {life: 150, dps: 0, fire_res: 15}
        ↓
   [Select node 3] → update_state() → {life: 180, dps: 50, fire_res: 15}
        ↓
...
| Variable | Source | Usage | Status |
|----------|--------|-------|--------|
| Diversity score | Heuristic | Temporary reward | ⚠️ Placeholder |
| Size score | Node count | Temporary reward | ⚠️ Placeholder |
...
**Data Flow:**
```
build_stats + encounter → FightSimulator.run() → outcome_vars
                                                        ↓
                                            compute_reward(outcome_vars)
                                                        ↓
                                            REINFORCE: -(log_probs × reward).sum()
```

---

## ⚔️ 5. Adversarial Variables
...
```
EncounterGenerator.sample_encounter() → encounter_1
                                              ↓
...
| Variable | Source | Usage | Status |
|----------|--------|-------|--------|
| Temperature | Manual parameter | Softmax temperature in sampling | ✅ Working |
| Greedy selection | argmax() | Deterministic best choice | ✅ Working |
...
**Data Flow:**
```
Generate tree → compute_novelty_score() → novelty_reward
                                               ↓
...
| Variable | Source | Usage | Status |
|----------|--------|-------|--------|
| Node embeddings | GCN layers | Learned representations | ✅ Working |
| Edge index | Real tree | Graph structure | ✅ Working |
...
1. SETUP
   ┌─────────────────┐
   │ Structural Vars │ edge_index.json → GNN
   └─────────────────┘
   ┌─────────────────┐
   │ Feature Vars    │ tree.lua → node_stats dict
   └─────────────────┘
   ┌─────────────────┐
   │ Exploration Mgr │ Initialize history, visit counts
   └─────────────────┘

2. GENERATE TREE
   ┌──────────────────┐
   │ Sample context   │ → gear, gems, skill, class
   └──────────────────┘
            ↓
   ┌──────────────────┐
   │ Initialize state │ → {life: base, dps: 0, res: 0}
   └──────────────────┘
            ↓
   ┌────────────────────────────────────────┐
   │ Greedy Generation Loop (N nodes)       │
   │                                        │
   │ For each step:                         │
   │   1. Get valid candidates (adjacent)   │  ← Structural Vars
   │   2. Encode context + state            │  ← State Vars
   │   3. Score candidates with GNN         │  ← Fitting Vars (embeddings)
   │   4. Select node (greedy or sample)    │  ← Exploration Vars (temperature)
   │   5. Update state                      │  ← Feature Vars (node stats)
   │   6. Track log probability             │  ← For REINFORCE
   └────────────────────────────────────────┘
            ↓
...
3. EVALUATE TREE
   ┌─────────────────────────┐
   │ BuildStatsDecoder       │
   │ allocated_nodes → stats │  ← Feature Vars (aggregate)
   └─────────────────────────┘
            ↓
   ...
   ┌─────────────────────────┐
   │ EncounterGenerator      │
   │ Sample/generate boss    │  ← Adversarial Vars
   └─────────────────────────┘
            ↓
   ...
   ┌─────────────────────────┐
   │ FightSimulator          │
   │ Simulate fight          │  ← Outcome Vars (compute)
   └─────────────────────────┘
            ↓
...
4. COMPUTE REWARD
   ┌─────────────────────────┐
   │ Performance reward      │  ← Outcome Vars (from result)
   │ + Novelty bonus         │  ← Exploration Vars
   │ + Exploration bonus     │  ← Exploration Vars
   └─────────────────────────┘
            ↓
...
5. UPDATE MODEL
   ┌─────────────────────────┐
   │ REINFORCE               │
   │ loss = -(log_probs      │  ← Tracked during generation
   │         × reward).sum() │  ← From step 4
   └─────────────────────────┘
            ↓
...
6. TRACK EXPLORATION
   ...
   │ Update visit counts     │
   │ Check for plateau       │
   │ Adjust temperature      │  ← Exploration Vars (adaptive)
   └─────────────────────────┘
...
|----------|---------------|---------------|--------------|
| **Structural** | ✅ 80% done | Extract node types | `parse_node_types.py` |
| **Feature** | ❌ 10% done | **BuildStatsDecoder** | `training/build_stats_decoder.py` |
| **State** | ⚠️ 40% done | **State tracking in PathGenerator** | `models/graph_tree_builder.py` (enhance) |
| **Outcome** | ❌ 0% done | **FightSimulator** | `training/fight_simulator.py` |
| **Adversarial** | ❌ 0% done | **EncounterGenerator** | `training/encounter_generator.py` |
| **Exploration** | ⚠️ 30% done | **ExplorationManager** | `training/exploration.py` |
| **Fitting** | ✅ 60% done | Optional: BuildEncoder | `models/build_encoder.py` (optional) |
...
1. **Structural + Feature = Understanding**
   - Structural variables → graph geometry
   - Feature variables → what nodes do
   - Combined → GNN learns meaningful patterns

2. **State = Context-Aware Decisions**
   - Without state: Greedy selection is blind
   - With state: Model knows what build needs next
   - Example: Low life → prioritize +life nodes
...
5. **Exploration = Avoiding Meta Collapse**
   - Pure performance → single meta build
   - Exploration bonus → diverse builds
   - Balance is critical
...
**Current Status:** Tree generation works, but rewards are placeholders. Priority: Build real evaluation pipeline (Feature → Outcome variables).
