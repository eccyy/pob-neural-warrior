...
local TreeTabClass = newClass("TreeTab", "ControlHost", function(self, build)
	...
	-- OPTIMIZER BUTTONS ROW (below the search bar)
	-- Set Optimizer Nodes Button
	self.controls.setOptimizerNodes = new("ButtonControl", { "TOPLEFT", self.controls.treeSearch, "BOTTOMLEFT" }, { 0, 8, 90, 20 }, "Set Nodes", function()
		self:SetOptimizerNodesPopup()
	end)
	self.controls.setOptimizerNodes.tooltipText = "Manually enter node IDs to allocate"
	
	-- Generate Best Nodes Button
	self.controls.generateBestNodes = new("ButtonControl", { "LEFT", self.controls.setOptimizerNodes, "RIGHT" }, { 8, 0, 110, 20 }, "Generate Best", function()
		self:GenerateBestNodesPopup()
	end)
	self.controls.generateBestNodes.tooltipText = "Automatically find optimal nodes using greedy algorithm"
	
	-- Apply Optimizer Button
	self.controls.applyOptimizer = new("ButtonControl", { "LEFT", self.controls.generateBestNodes, "RIGHT" }, { 8, 0, 100, 20 }, "Apply Nodes", function()
		self:ApplyOptimizerSuggestions()
	end)
	self.controls.applyOptimizer.tooltipText = "Allocate the suggested nodes to your tree"
	
	-- GPU Optimize Button (External) - For neural network / ML-based optimization
	self.controls.gpuOptimize = new("ButtonControl", { "LEFT", self.controls.applyOptimizer, "RIGHT" }, { 8, 0, 110, 20 }, "GPU Optimize", function()
		self:GPUOptimizeExternal()
	end)
	self.controls.gpuOptimize.tooltipText = "Neural network optimizer (placeholder)\nUse 'Generate Best' for working optimization"

	--Default index for Tattoos
	self.defaultTattoo = { }

	-- Show Node Power Checkbox
	self.controls.treeHeatMap = new("CheckBoxControl", { "LEFT", self.controls.gpuOptimize, "RIGHT" }, { 8, 0, 20 }, "Show Node Power:", function(state)
...
function TreeTabClass:GenerateBestNodesPopup()
	local controls = { }
	
	controls.label = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 20, 0, 16 }, "^7Select stat to optimize and number of nodes:")
	
	controls.statLabel = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 50, 0, 16 }, "^7Stat:")
	controls.statSelect = new("DropDownControl", { "TOPLEFT", nil, "TOPLEFT" }, { 70, 48, 250, 20 }, self.powerStatList, function(index, value)
		-- Stat selection changed
	end)
	controls.statSelect.selIndex = 1
	controls.statSelect.tooltipFunc = function(tooltip, mode, index, value)
		tooltip:Clear()
		if mode ~= "OUT" and value and value.label then
			tooltip:AddLine(16, "^7"..value.label)
			if value.combinedDPS then
				tooltip:AddLine(14, "^7Combines offence and defence metrics")
			end
		end
	end
	
	controls.countLabel = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 80, 0, 16 }, "^7Number of nodes:")
	controls.countEdit = new("EditControl", { "TOPLEFT", nil, "TOPLEFT" }, { 150, 78, 60, 20 }, "10", nil, "%D", 3)
	controls.countEdit.tooltipText = "Number of best nodes to suggest (1-50)"
	
	controls.help = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 110, 0, 14 }, "^7This will calculate node power and select the best nodes.")
	controls.warning = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 130, 0, 14 }, "^1Warning: This may take a few seconds to calculate.")
	
	-- Check GPU availability
	local pythonBridge = self.build.calcsTab.pythonBridge
	local gpuAvailable = pythonBridge and pythonBridge.isAvailable()
	local gpuStatusText = gpuAvailable and "^2GPU acceleration: Available" or "^1GPU acceleration: Not available (will use CPU)"
	controls.gpuStatus = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 150, 0, 14 }, gpuStatusText)
	
	controls.generate = new("ButtonControl", nil, { -90, 180, 80, 20 }, "Generate", function()
		local count = tonumber(controls.countEdit.buf) or 10
		if count < 1 or count > 50 then
			main:OpenMessagePopup("Error", "Number of nodes must be between 1 and 50.")
			return
		end
		
		local selectedStat = controls.statSelect.list[controls.statSelect.selIndex]
		if not selectedStat then
			main:OpenMessagePopup("Error", "Please select a stat.")
			return
		end
		
		main:ClosePopup()
		
		-- Set the power stat and trigger calculation
		self:SetPowerCalc(selectedStat)
		self.build.calcsTab.powerBuildFlag = true
		
		-- Show a message that calculation is in progress
		main:OpenMessagePopup("Optimizer", "Calculating node power... Please wait.")
		
		-- Set a callback to collect best nodes when calculation completes
		self.build.powerBuilderCallback = function()
			-- Clear callback immediately to prevent re-execution
			self.build.powerBuilderCallback = nil
			-- Close the waiting message
			main:ClosePopup()
			-- Safety check before calling CollectBestNodes
			if not self.build or not self.build.spec then
				main:OpenMessagePopup("Error", "Build not properly initialized. Please try again.")
				return
			end
			self:CollectBestNodes(count, selectedStat)
		end
	end)
	
	controls.cancel = new("ButtonControl", nil, { 0, 180, 80, 20 }, "Cancel", function()
		main:ClosePopup()
	end)
	
	main:OpenPopup(340, 220, "Generate Best Nodes", controls, "generate")
end

function TreeTabClass:CollectBestNodes(count, selectedStat)
	local spec = self.build.spec
	local totalNodesUsed = 0
	local selectedNodes = nil  -- Will be set by Python or Lua optimizer
	local tempAllocated = { }
	
	-- Count currently allocated nodes
	for nodeId, node in pairs(spec.allocNodes) do
		if node.type ~= "ClassStart" and node.type ~= "AscendClassStart" and not node.ascendancyName then
			totalNodesUsed = totalNodesUsed + 1
		end
		tempAllocated[nodeId] = true
	end
	
	ConPrintf("Optimizer: Starting with %d nodes allocated, budget %d", totalNodesUsed, count)
	
	-- Count available nodes for debugging
	local availableCount = 0
	local nodesWithPower = 0
	local nodesWithPath = 0
	local nodesPassAllFilters = 0
	for nodeId, node in pairs(spec.nodes) do
		if not tempAllocated[nodeId] and not node.ascendancyName and node.type ~= "ClassStart" then
			availableCount = availableCount + 1
			local hasPower = node.power and node.power.singleStat and node.power.singleStat > 0
			local hasPath = node.path ~= nil
			
			if hasPower then
				nodesWithPower = nodesWithPower + 1
			end
			if hasPath then
				nodesWithPath = nodesWithPath + 1
			end
			
			-- Check if node would pass all filters (including granted passives check)
			if hasPower and hasPath and not self.build.calcsTab.mainEnv.grantedPassives[nodeId] then
				nodesPassAllFilters = nodesPassAllFilters + 1
			end
		end
	end
	ConPrintf("Optimizer: %d unallocated nodes, %d with power>0, %d with path, %d pass all filters", availableCount, nodesWithPower, nodesWithPath, nodesPassAllFilters)
	
	-- Early exit with clear message if data is missing
	if nodesWithPath == 0 or nodesWithPower == 0 then
		local debugMsg = string.format([[Cannot optimize - missing data:

Total unallocated nodes: %d
Nodes with power > 0: %d  
Nodes with path data: %d

You need to enable "Show Node Power" first to calculate node values!
1. Check the "Show Node Power" checkbox at the top
2. Wait for calculation to complete
3. Then click "Generate Best" again]], 
			availableCount, nodesWithPower, nodesWithPath)
		
		main:OpenMessagePopup("Optimizer - Missing Data", debugMsg)
		return
	end
	
	-- Debug: Check a few nodes in detail
	local sampleCount = 0
	for nodeId, node in pairs(spec.nodes) do
		if not tempAllocated[nodeId] and not node.ascendancyName and node.type ~= "ClassStart" and sampleCount < 3 then
			sampleCount = sampleCount + 1
			ConPrintf("Sample node %d: power=%s, path=%s, pathLen=%d", nodeId, 
				tostring(node.power and node.power.singleStat), 
				tostring(node.path ~= nil),
				node.path and #node.path or 0)
		end
	end
	
	-- Try GPU-accelerated Python optimizer first
	local pythonBridge = self.build.calcsTab.pythonBridge
	if pythonBridge and pythonBridge.isAvailable() then
		-- Prepare node data for Python
		local nodeDataList = { }
		for nodeId, node in pairs(spec.nodes) do
			if not tempAllocated[nodeId] and node.path and node.power and node.power.singleStat 
				and node.power.singleStat > 0 and not node.ascendancyName 
				and node.type ~= "ClassStart" and not self.build.calcsTab.mainEnv.grantedPassives[nodeId] then
				
				-- Calculate path cost
				local pathCost = 0
				local pathIds = { }
				for _, pathNode in ipairs(node.path) do
					t_insert(pathIds, pathNode.id)
					if not tempAllocated[pathNode.id] then
						pathCost = pathCost + 1
					end
				end
				
				t_insert(nodeDataList, {
					nodeId = nodeId,
		...
		-- Call Python GPU optimizer
		local gpuResult = pythonBridge.gpu_optimize_tree(nodeDataList, count, totalNodesUsed)
		if gpuResult and #gpuResult > 0 then
			-- Python succeeded, use its result
			selectedNodes = { }
			for _, nodeId in ipairs(gpuResult) do
				t_insert(selectedNodes, nodeId)
			end
			ConPrintf("Optimizer: Used GPU-accelerated Python optimizer")
			main:OpenMessagePopup("Optimizer", string.format("\n^2✓ GPU ACCELERATION ACTIVE\n\n^7Found %d optimal nodes using GPU-accelerated calculations.\n\nClick 'Apply Nodes' to allocate them to your tree.", #selectedNodes))
		else
			-- Python failed or unavailable, fallback to Lua
			ConPrintf("Optimizer: GPU optimizer failed, using Lua fallback")
			selectedNodes = nil
		end
	end
	
	-- Lua fallback: Greedy selection if Python didn't work
	if not selectedNodes then
		selectedNodes = { }
		local iterationCount = 0
		while totalNodesUsed < count do
			iterationCount = iterationCount + 1
			if iterationCount > count * 2 then
				ConPrintf("Loop safety break at iteration %d", iterationCount)
				break
			end
			...
			-- Find the best unallocated node considering path efficiency
			for nodeId, node in pairs(spec.nodes) do
				if not tempAllocated[nodeId] and node.path and node.power and node.power.singleStat 
					and node.power.singleStat > 0 and not node.ascendancyName 
					and node.type ~= "ClassStart" and not self.build.calcsTab.mainEnv.grantedPassives[nodeId] then
					...
					if totalNodesUsed + pathCost <= count then
						-- Calculate efficiency: power per point spent
						local efficiency = node.power.singleStat / m_max(pathCost, 1)
			...
			-- If we found no valid node, we're done
			if not bestNode then
				break
			end
			
			-- "Allocate" this node and its path temporarily
			for _, pathNode in ipairs(bestNode.path) do
				if not tempAllocated[pathNode.id] then
					tempAllocated[pathNode.id] = true
					totalNodesUsed = totalNodesUsed + 1
				end
			end
			
			-- Store only the target node
			t_insert(selectedNodes, bestNode.id)
		end
		
		-- Check if we found any nodes (before showing popup)
		if #selectedNodes == 0 then
			-- Show detailed debug info about why no nodes were found
			local debugMsg = string.format([[No valid nodes found within budget of %d nodes.

Debug info:
- Currently allocated: %d nodes
- Total unallocated nodes: %d
- Nodes with power > 0: %d  
- Nodes with path data: %d

This usually means:
1. Node power calculation didn't run (click Show Node Power first)
2. Budget is too small (all remaining paths exceed %d points)
3. No nodes are reachable from your current position]], 
				count, totalNodesUsed, availableCount, nodesWithPower, nodesWithPath, count)
			
			main:OpenMessagePopup("Optimizer Debug", debugMsg)
			return
		end
		
		-- Show success message (no GPU warning since Lua is fast enough)
		main:OpenMessagePopup("Optimizer", string.format("Found %d optimal nodes within %d point budget.\n\nClick 'Apply Nodes' to allocate them to your tree.", #selectedNodes, count))
	end
	
	-- Store the selected nodes (works for both GPU and CPU paths)
	self.build.calcsTab:StoreOptimizerSuggestions(selectedNodes)
end

function TreeTabClass:SetOptimizerNodesPopup()
	local controls = { }
	local nodeIdText = ""
	
	-- Load existing suggestions if any
	if self.build.calcsTab.optimizerSuggestions and #self.build.calcsTab.optimizerSuggestions > 0 then
		local ids = { }
		for _, id in ipairs(self.build.calcsTab.optimizerSuggestions) do
			t_insert(ids, tostring(id))
		end
		nodeIdText = table.concat(ids, ", ")
	end
	
	controls.label = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 20, 0, 16 }, "^7Enter passive node IDs (comma or space separated):")
	controls.edit = new("EditControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 45, 440, 60 }, nodeIdText, nil, nil, nil, nil, 14)
	controls.edit.prompt = "Example: 36634, 61419, 2491, 61305, 24970"
	
	controls.help = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 115, 0, 14 }, "^7Tip: You can find node IDs by hovering over nodes in the tree")
	
	controls.save = new("ButtonControl", nil, { -90, 145, 80, 20 }, "Save", function()
		local input = controls.edit.buf
		if input == "" then
			main:OpenMessagePopup("Error", "Please enter at least one node ID.")
			return
		end
		
		-- Parse the input (comma, space, or newline separated)
		local nodeIds = { }
		for id in input:gmatch("%d+") do
			local nodeId = tonumber(id)
			if nodeId and self.build.spec.nodes[nodeId] then
				t_insert(nodeIds, nodeId)
			end
		end
		
		if #nodeIds == 0 then
			main:OpenMessagePopup("Error", "No valid node IDs found. Make sure the nodes exist in the passive tree.")
			return
		end
		
		self.build.calcsTab:StoreOptimizerSuggestions(nodeIds)
		main:ClosePopup()
		main:OpenMessagePopup("Optimizer", #nodeIds .. " node(s) saved. Click 'Apply Nodes' to allocate them.")
	end)
	
	controls.cancel = new("ButtonControl", nil, { 0, 145, 80, 20 }, "Cancel", function()
		main:ClosePopup()
	end)
	
	main:OpenPopup(480, 185, "Set Optimizer Nodes", controls, "save", "edit")
end

function TreeTabClass:ApplyOptimizerSuggestions()
	local suggestions = self.build.calcsTab.optimizerSuggestions
	local debugInfo = string.format("suggestions=%s, count=%d", tostring(suggestions ~= nil), suggestions and #suggestions or 0)
	-- Temporarily show debug info
	-- main:OpenMessagePopup("Debug Apply", debugInfo)
	if not suggestions or #suggestions == 0 then
		main:OpenMessagePopup("Optimizer", "No nodes to apply. Click 'Generate Best' first to find optimal nodes.")
		return
	end
	
	local allocated = 0
	for _, nodeId in ipairs(suggestions) do
	...
	if allocated > 0 then
		self.build.spec:AddUndoState()
		-- Force an immediate build calculation to update mastery counts and all state
		self.build.buildFlag = true
		self.build.calcsTab:BuildOutput()
		-- Clear any pending power builder callback and reset the coroutine to avoid dead coroutine errors
		self.build.powerBuilderCallback = nil
		self.build.calcsTab.powerBuilder = nil
		self.build.calcsTab.powerBuildFlag = false
		-- Turn off heat map to prevent trying to resume dead coroutine
		self.viewer.showHeatMap = false
		self.controls.treeHeatMap.state = false
		self.controls.treeHeatMapStatSelect.shown = false
		self.controls.powerReportList.shown = false
		-- Disable stat differences in tooltips to avoid mastery count calculation errors
		self.viewer.showStatDifferences = false
		main:OpenMessagePopup("Optimizer", allocated .. " nodes allocated successfully.")
	else
		main:OpenMessagePopup("Optimizer", "All suggested nodes were already allocated.")
	end
end

function TreeTabClass:GPUOptimizeExternal()
	-- Show popup to configure external GPU optimization
	local controls = { }
	
	controls.label = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 20, 0, 16 }, "^7GPU-Accelerated Optimization (External)")
	controls.help = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 45, 0, 14 }, "^7This will export your build and run optimization via Python/PyTorch.")
	
	controls.countLabel = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 75, 0, 16 }, "^7Number of nodes:")
	controls.countEdit = new("EditControl", { "TOPLEFT", nil, "TOPLEFT" }, { 150, 73, 60, 20 }, "40", nil, "%D", 3)
	controls.countEdit.tooltipText = "Number of nodes to optimize (1-100)"
	
	-- Check if Python is available
	controls.status = new("LabelControl", { "TOPLEFT", nil, "TOPLEFT" }, { 20, 105, 0, 14 }, "^7Checking Python availability...")
	
	controls.run = new("ButtonControl", nil, { -90, 135, 80, 20 }, "Optimize", function()
		local count = tonumber(controls.countEdit.buf) or 40
		if count < 1 or count > 100 then
			main:OpenMessagePopup("Error", "Number of nodes must be between 1 and 100.")
			return
		end
		
		main:ClosePopup()
		self:RunGPUOptimizerExternal(count)
	end)
	
	controls.cancel = new("ButtonControl", nil, { 0, 135, 80, 20 }, "Cancel", function()
		main:ClosePopup()
	end)
	
	main:OpenPopup(400, 175, "GPU Optimize", controls, "run")
	
	-- Check Python availability asynchronously (simplified - just show the popup)
	controls.status.label = "^2Ready to optimize. Click 'Optimize' to start."
end

function TreeTabClass:RunGPUOptimizerExternal(count)
	-- Show info message about neural network placeholder
	local msg = string.format([[GPU Neural Network Optimizer (Coming Soon)

This feature is a placeholder for future ML-based optimization.

Current Status:
• Greedy optimizer: Use "Generate Best" button (working!)
• Neural network: Not yet implemented

The neural network will be trained on thousands of builds to learn optimal passive tree patterns and provide better recommendations than greedy algorithms.

For now, use "Generate Best" which already gives ~50%% better results than manual allocation.]])
	
	main:OpenMessagePopup("GPU Optimizer", msg)
end
